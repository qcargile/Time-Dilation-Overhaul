...
A fragment lets you trigger dilation while driving. The car doesn't go faster, but grip and turning improve significantly. 
...
Each manufacturer's operating system is its own playstyle. 
...
The deepest dive in the lineup. Slows time by 99% (you aren't slowed), with bonus move speed and infinite stamina, and records your path. On expiry it rewinds you back along that path and releases every attack you lined up all at once, then leaves you staggered from the whiplash. Pure burst, paid for in recovery.
...
Slows time, you aren't slowed. Your weapons fire several times faster than their mechanical limit, with no crits or weakspot damage. Each hit ramps damage up toward +100%, and a miss resets the ramp to zero. Recoil climbs hard as the price. (Reflexes-attuned)
...
Slows time by 99%, and you're slowed too. While slowed, aim a teleport marker and warp to it; you can't act until you commit or cancel. On arrival it implants Quantum Malware into the nearest enemies, freezing each of them in their own dilated time. The **Advanced** model banks two teleport charges back to back. (Cool-attuned)

### Arasaka "Sogimsu" - Watchdog Protocol
No time dilation. A reactive counter-detection AI runs in the background and auto-quickhacks (Memory Wipe + Reboot Optics) any hostile about to spot you. It ends early if you're pulled into combat, and drops you into an empowered Optical Camo when it ends so you can reposition. (Cool-attuned.)

### Anvil Defense "Juggernaut" - Armor Lock
No time dilation. Lock down to absorb a set amount of incoming damage with full status immunity. You can't act while locked. End it early or let it auto-release at the cap, and the stored damage detonates as a 360° kinetic shockwave that knocks down everyone nearby. (Body-attuned)
...
- Most changes don't apply until you reload a save / the gamne.
- The ESR (enemy) options live in their own subsection under the same tab.
- No Native Settings UI? Edit `bin\x64\plugins\cyber_engine_tweaks\mods\tdo\config\userConfig.lua` 
